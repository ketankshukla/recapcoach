...
Phase 0 is the foundation. Visible UI changes start in **Phase 1 —
Home screen overhaul** ([08-roadmap.md](08-roadmap.md)):

- Refined empty state with subtle illustration
- Note cards with status chips + tactile shadows (using `surfaceContainer` + `outlineVariant`)
- Usage meter redesigned with gradient + animated fill (using
  `AppSemanticColors.usageMeterColorFor`)
- App bar with avatar + time-based greeting
- FAB with mic icon + gentle pulse animation (using
  `tokens.recordingPulse`)
- Skeleton loading states (using `tokens.shimmer`)

Every Phase 1 widget reads tokens through the patterns described in
section 9; nothing hard-coded.
