...
- **Phase 2 glass theme rollout — Paywall + Record + Settings shipped.**
  ...
  - Record screen sits over the same mesh: amber-gradient `AmplitudePulseMic` with halos that breathe with live amplitude, glass timer card with a recording dot, amber-gradient `AmplitudeWaveform`, red-tinted `GlassIconButton` for discard + `GradientPillButton` for Stop & save, and the shared `GlassAlertDialog` for permission / quota prompts.
  - Settings replaces the M3 `ListView`-of-`ListTile`s with mesh-backed sections: four `GlassCard`s (Account / Subscription / Legal & support / Account actions), each with a hand-rolled `_SettingsRow` so the divider, ripple, and tint match the glass aesthetic. Delete-account uses `GlassAlertDialog(primaryDestructive: true)` to swap the amber CTA for a red outlined button (chapter 13 § 12).
- Four shared primitives in `lib/core/widgets/glass/`: `GradientPillButton`, `GlassIconButton`, `GlassPillButton`, `GlassAlertDialog`. Two recording widgets in `lib/features/recording/widgets/`.
